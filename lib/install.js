...
function install(opts, cb) {
  ...
  if (fsPaths.edge) {
    tasks.push(setDriverFilePermissions.bind(null, fsPaths.edge.installPath));
  }
  ...
  function installEdgeDr(opts, cb) {
    if (path.extname(opts.from) === '.msi') {
      downloadInstallerFile(opts.form, opts.to, cb);
    } else {
      installSingleFile(opts.form, opts.to, cb);
    }
  }
  ...
  function downloadInstallerFile(from, to, cb) {
    getDownloadStream(from, function(err, stream) {
      ...
      // Store installer
      var msiWriteStream = fs.createWriteStream(to)
        .once('error', cb.bind(null, new Error('Could not write to ' + to)));
      stream.pipe(msiWriteStream);

      msiWriteStream.once('finish', runInstaller.bind(null, from, to, cb));
    });
  }

  function runInstaller(from, to, cb) {
    var logFile = 'install.log';
    var options = [
      '/passive',         // no user interaction, only show progress bar
      '/l* ' + logFile,   // save install log to this file
      '/i ' + from        // msi file to install
    ];
